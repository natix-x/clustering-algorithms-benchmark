...
@pytest.mark.parametrize("missing", ["name", "configs_dir", "log_dir", "output_dir", "jar_path", "matrix"])
...
@pytest.mark.parametrize("missing", ["nodes", "algorithm", "dataset"])
def test_missing_matrix_key_raises(spark_yaml, missing):
    del spark_yaml["matrix"][missing]
    with pytest.raises(KeyError):
        _validate(spark_yaml)


def test_repetitions_inside_matrix_raises(spark_yaml):
    spark_yaml["matrix"]["repetitions"] = [2]
...
def test_missing_resource_key_raises(spark_yaml):
    del spark_yaml["matrix"]["resources"][0]["worker_mem_gb"]
    with pytest.raises(KeyError):
        _validate(spark_yaml)


def test_matrix_resources_required(spark_yaml):
    del spark_yaml["matrix"]["resources"]
    with pytest.raises(KeyError):
        _validate(spark_yaml)
...
def test_resource_int_key_must_be_positive_int(spark_yaml, bad):
    spark_yaml["matrix"]["resources"][0]["cpus_per_task"] = bad
    with pytest.raises(ValueError):
        _validate(spark_yaml)
...
@pytest.mark.parametrize("axis,drop", [("dataset", "type"), ("dataset", "params"),
                                       ("algorithm", "name"), ("algorithm", "params")])
def test_entry_missing_required_key_raises(spark_yaml, axis, drop):
    del spark_yaml["matrix"][axis][0][drop]
    with pytest.raises(KeyError):
        _validate(spark_yaml)


@pytest.mark.parametrize("axis", ["dataset", "algorithm"])
def test_entry_params_must_be_dict(spark_yaml, axis):
    spark_yaml["matrix"][axis][0]["params"] = "oops"
    with pytest.raises(ValueError):
        _validate(spark_yaml)
...
def test_entry_must_be_dict(spark_yaml, axis):
    spark_yaml["matrix"][axis][0] = "notadict"
    with pytest.raises(ValueError):
        _validate(spark_yaml)
